...
a. Almacenar los productos vendidos en una estructura eficiente para la búsqueda por código de producto. De cada producto deben quedar almacenados su código, la cantidad total de unidades vendidas y el monto total. 
De cada venta se cargan código de venta, código del producto vendido, cantidad de unidades vendidas y precio unitario. El ingreso de las ventas finaliza cuando se lee el código de venta 0.
...
procedure ModuloD (a: arbol; var cantidad:integer);
{ Retornar la cantidad de códigos que existen en el árbol que son menores que un valor que se recibe como parámetro }
  ...
  begin 
       if (a=nil) then CantidadDeCodigosMenores:=0;
        if (a<>nil) then 
            begin
                   if  (a<>nil) and  ( cod > a^.dato.codigo )  then   CantidadDeCodigosMenores:=  1   +   CantidadDeCodigosMenores (a^.HI,cod)  +  CantidadDeCodigosMenores (a^.HD,cod)  
                   else if  (a<>nil) and (cod <= a^.dato.codigo ) then CantidadDeCodigosMenores := CantidadDeCodigosMenores(a^.HI,cod) 
             end;
end;
                                   
var  unCodigo: integer;
begin
  writeln;
  writeln ('----- Modulo D ----->');
  writeln;
...
procedure ModuloE (a: arbol; var montoT:real);
...
begin
    ...
    if ( a<> nil) then
           begin
                if ( a<> nil) and (codigo1 < a^.dato.codigo) and  ( a^.dato.codigo < codigo2)  then   ObtenerMontoTotalEntreDosCodigos:= a^.dato.montoTotal +  ObtenerMontoTotalEntreDosCodigos(a^.HI, codigo1,codigo2) + ObtenerMontoTotalEntreDosCodigos(a^.HD, codigo1,codigo2)
                else if (codigo1 >= a^.dato.codigo ) then  ObtenerMontoTotalEntreDosCodigos:=  ObtenerMontoTotalEntreDosCodigos(a^.HD, codigo1,codigo2)
                else if (codigo2 <= a^.dato.codigo ) then  ObtenerMontoTotalEntreDosCodigos:=  ObtenerMontoTotalEntreDosCodigos(a^.HI, codigo1,codigo2);
            end;      
end;
...
begin
  ...
  readln (codigo2);
  writeln;
  montoT:= ObtenerMontoTotalEntreDosCodigos (a, codigo1, codigo2);
  if (montoT = 0) 
  then writeln ('No hay codigos entre ', codigo1, ' y ', codigo2)
  else begin
         writeln;
         writeln ('El monto total entre el codigo', codigo1, ' y el codigo : ', codigo2, ' es: ', montoT:0:2); 
         writeln;
       end;
  writeln;
  writeln ('-----------------------------------------------');
...
var a: arbol;   cantidad:integer; montoT:real;
Begin
  cantidad:= 0;
  randomize;
  ModuloA (a);
